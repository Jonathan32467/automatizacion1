...
for i in $(seq 1 $TOTAL_FILES); do
    ...
    echo "[$(date +%T)] Uso actual: $(df -h "$DEST_DIR" | awk 'NR==2 {print $3 " usados / " $2 " total (" $5 " usado)"}')" | tee -a "$LOG_FILE"

    sleep 2
done

echo ""
echo "Simulación completada. Archivos en: $DEST_DIR"
echo "Uso final del disco:"
df -h "$DEST_DIR"
